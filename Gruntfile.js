...
module.exports = function (grunt) {
    ...
    if (process.argv[2] == 'rename') {
        ...
        if (!newname) {
            console.log('Please write the new sma-speedwire name, like: "grunt rename --name=mywidgetset" --author="Author Name"');
            process.exit();
        }
        ...
        if (fs.existsSync(__dirname + '/admin/sma-speedwire.png')) {
            fs.renameSync(__dirname + '/admin/sma-speedwire.png',              __dirname + '/admin/' + newname + '.png');
        }
        if (fs.existsSync(__dirname + '/widgets/sma-speedwire.html')) {
            fs.renameSync(__dirname + '/widgets/sma-speedwire.html',           __dirname + '/widgets/' + newname + '.html');
        }
        if (fs.existsSync(__dirname + '/widgets/sma-speedwire/js/sma-speedwire.js')) {
            fs.renameSync(__dirname + '/widgets/sma-speedwire/js/sma-speedwire.js', __dirname + '/widgets/sma-speedwire/js/' + newname + '.js');
        }
        if (fs.existsSync(__dirname + '/widgets/sma-speedwire')) {
            fs.renameSync(__dirname + '/widgets/sma-speedwire',                __dirname + '/widgets/' + newname);
        }
    }
        ...
        replace: {
            ...
            name: {
                options: {
                    patterns: [
                        {
                            match: /sma-speedwire/g,
                            replacement: newname
                        },
                        {
                            match: /Sma-speedwire/g,
                            replacement: newname ? (newname[0].toUpperCase() + newname.substring(1)) : 'Sma-speedwire'
                        },
                        {
                            match: /@@Author@@/g,
                            replacement: author
